...
def run_conditionally(
    ...
    **kwargs,
    ...
    """
    Main entry point of the Qonscious runner.
    ...
    ----------
    backend_adapter : BackendAdapter
        Adapter to the quantum backend on which figures of merit are
        evaluated and circuits may be executed.
    checks : list[MeritComplianceCheck]
        The merit compliance checks to perform on the backend.
    on_pass : QonsciousAction
        Action to execute if all checks succeed. Must implement
        `run(backend_adapter, fom_results, **kwargs)` and return an
        `ExperimentResult` or `None`.
    on_fail : QonsciousAction
        Action to execute if any check fails. Same contract as `on_pass`.
    **kwargs
        Additional keyword arguments forwarded to the checks and actions.

    Returns
    -------
    QonsciousResult
        A result object containing the list of figure-of-merit results
